...
\begin{document}
  ...
  \begin{subquestion}{11.9}
    One problem is that a process could acquire a right lock to the new file in the same location but a different process could acquire a write lock to the old file in the same location,
  ...
  \begin{subquestion}{11.12}
    a text editor is an example of an application that would access files sequentially because it reads the files from start to end in order every time in order to display them properly.
  ...
  \begin{subquestion}{12.13}
    the obvious benefit of this system would be a reduction in integral fragmentation since you could more closely allocate the exact amount of space that you would need to store your file.
