...
exp returns [Exp ast]:
      ...
      | array=arrayexp {$ast = $array.ast;}
        | index=indexexp {$ast = $index.ast;}
        | arrayassign=arrayassignexp {$ast=$arrayassign.ast;}
    	;
  ...
  indexexp returns [IndexExp ast]
      locals [ArrayList<Exp> list = new ArrayList<Exp>()]:
        '(' 'index'
            e=exp
            ( e2=exp {$list.add($e2.ast);})+
        ')' { $ast = new IndexExp($e.ast, $list); }
        ;

  arrayassignexp returns [ArrayAssignExp ast]
        locals [ArrayList<Exp> list = new ArrayList<Exp>()] :
        '(' 'assign'
            a=exp
            ( e=exp { $list.add($e.ast); } )+
            v=exp
        ')' { $ast = new ArrayAssignExp($a.ast, $list, $v.ast); }
        ;
