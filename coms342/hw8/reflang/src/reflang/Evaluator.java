...
public class Evaluator implements Visitor<Value> {
    ...
    boolean ref = false;
      ...
      if(intermediate instanceof Value.RefVal){
        ref = true;
        result += (double)((Value.RefVal)intermediate).loc();
      } else{
			  result += ((Value.NumVal)intermediate).v(); //Semantics of AddExp in terms of the target language.
      }
		}
    if(ref){
      return new RefVal((int)result);
    }
    ...
    if(lVal instanceof NumVal)
      ...
      if(rVal instanceof NumVal)
        ...
        result = result - (double)((RefVal)rVal).loc();
        ref = true;
      }
		}
    if(ref){
    ...
    Value t;
		Exp value_exp = e.value_exp();
		Value value = (Value) value_exp.accept(this, env);
    //System.out.println(e.accept(ts, env));
		t = heap.ref(value);
    if(t instanceof Value.DynamicError){
    ...
    if(loc instanceof Value.RefVal)
		  return heap.deref((Value.RefVal)loc);
    else if(loc instanceof Value.NumVal)
      return heap.deref((Value.NumVal)loc);
    else
      return loc;
	}
	...
	public Value visit(AssignExp e, Env env) { // New for reflang.
		...
		Value assign_val;
    if(loc instanceof Value.RefVal)
      assign_val = heap.setref((Value.RefVal)loc, rhs_val);
    else if(loc instanceof Value.NumVal)
      assign_val = heap.setref((Value.NumVal)loc, rhs_val);
    else
      return loc;
		return assign_val;
	}
  ...
  public Value visit(ArrayExp e, Env env){
    ArrayList<Exp> list = e.getDimensions();
    Value v;
    int args[] = new int[3];
    args[0] = 1;
    args[1] = 1;
    args[2] = 1;
    if(list.size() > 3)
      return new Value.DynamicError("arrays of size 4 and above aren't permitted.");
    if(list.size() < 1)
      return new Value.DynamicError("missing dimension arguments for array");
    for(int i = 0; i < list.size(); i++){
      v = (Value) list.get(i).accept(this, env);
      if(! (v instanceof Value.NumVal))
        return new Value.DynamicError("arguments for array demensions must resolve to a numeric value");
      else
        args[i] = (int)((NumVal) v).v();
    }
    return new ArrayVal(args[0], args[1], args[2]);
  }
  ...
  public Value visit(IndexExp e, Env env){
    ...
    ArrayList<Exp> list;
    int[] in = {0, 0, 0};
    v = (Value) e.getArray().accept(this, env);
    list = e.getIndex();
    if(!(v instanceof Value.ArrayVal))
      return new Value.DynamicError("first argument must resolve to an array");
    if(list.size() != ((ArrayVal)v).getDim())
      return new Value.DynamicError("wrong number of indices for the given array");
    for(int i = 0; i < list.size(); i++){
      t = (Value) list.get(i).accept(this, env);
      if(!(t instanceof Value.NumVal))
        return new Value.DynamicError("index arguments must resolve to a numerical value");
      in[i] = (int)((NumVal) t).v() - 1;
    }
    return ((ArrayVal)v).get(in[0], in[1], in[2]);
  }

  @Override
  public Value visit(ArrayAssignExp e, Env env){
    Value array, temp, value;
    ArrayList<Exp> list;
    int[] in = {0, 0, 0};
    array = (Value) e.getArray().accept(this, env);
    list = e.getIndex();
    value = (Value) e.getValue().accept(this, env);
    if(!(array instanceof Value.ArrayVal))
      return new Value.DynamicError("first argument must resolve to an array");
    if(!(value instanceof Value.NumVal))
      return new Value.DynamicError("can only assign numerical values to the array");
    if(list.size() != ((ArrayVal)array).getDim())
      return new Value.DynamicError("wrong number of indices for the given array");
    for(int i = 0; i < list.size(); i++){
      temp = (Value) list.get(i).accept(this, env);
      if(!(temp instanceof Value.NumVal))
        return new Value.DynamicError("index arguments must resolve to a numerical value");
      in[i] = (int)((NumVal) temp).v() - 1;
    }
    ((ArrayVal)array).set(in, (NumVal)value);
    return array;
  }
